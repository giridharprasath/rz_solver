...
#define mu_assert_true(actual, message)                                        \
  do {                                                                         \
    ...
    if (!(act__)) {                                                            \
      ...
      snprintf(_meqstr, MU_BUF_SIZE, "%s: expected true, got false",           \
               (message));                                                     \
...
#define mu_assert_false(actual, message)                                       \
  do {                                                                         \
    ...
    if ((act__)) {                                                             \
      char _meqstr[MU_BUF_SIZE];                                               \
      snprintf(_meqstr, MU_BUF_SIZE, "%s: expected false, got true",           \
               (message));                                                     \
      mu_assert(_meqstr, false);                                               \
    }                                                                          \
  } while (0)
  ...
  do {                                                                         \
    ...
    if ((exp__) != (act__)) {                                                  \
      ...
      snprintf(_meqstr, MU_BUF_SIZE,                                           \
               "%s: expected %" PFMT64d ", got %" PFMT64d ".", (message),      \
               (ut64)(exp__), (ut64)(act__));                                  \
      mu_assert(_meqstr, false);                                               \
    }                                                                          \
  } while (0)
...
#define mu_assert_eqf(actual, expected, message)                               \
  do {                                                                         \
    ...
    if ((exp__) != (act__)) {                                                  \
      char _meqstr[MU_BUF_SIZE];                                               \
      snprintf(_meqstr, MU_BUF_SIZE, "%s: expected %lf, got %lf.", (message),  \
               exp__, act__);                                                  \
      mu_assert(_meqstr, false);                                               \
    }                                                                          \
  } while (0)
...
#define mu_assert_neq(actual, expected, message)                               \
  do {                                                                         \
    ...
    ut64 exp__ = (ut64)(expected);                                             \
    snprintf(_meqstr, MU_BUF_SIZE,                                             \
             "%s: expected not %" PFMT64d ", got %" PFMT64d ".", (message),    \
             (exp__), (act__));                                                \
...
#define mu_assert_ptreq(actual, expected, message)                             \
  do {                                                                         \
    ...
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected %p, got %p.", (message),      \
             (exp__), (act__));                                                \
    mu_assert(_meqstr, (exp__) == (act__));                                    \
  } while (0)
...
#define mu_assert_ptrneq(actual, expected, message)                            \
  do {                                                                         \
    ...
    const void *act__ = (actual);                                              \
    const void *exp__ = (expected);                                            \
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected not %p, got %p.", (message),  \
             (exp__), (act__));                                                \
    mu_assert(_meqstr, (exp__) != (act__));                                    \
  } while (0)
...
#define mu_assert_null(actual, message)                                        \
  do {                                                                         \
    char _meqstr[MU_BUF_SIZE];                                                 \
    const void *act__ = (actual);                                              \
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected to be NULL but it wasn't.",   \
             (message));                                                       \
...
#define mu_assert_notnull(actual, message)                                     \
  do {                                                                         \
    char _meqstr[MU_BUF_SIZE];                                                 \
    const void *act__ = (actual);                                              \
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected to not be NULL but it was.",  \
             (message));                                                       \
    mu_assert(_meqstr, (act__) != NULL);                                       \
  } while (0)
...
#define mu_assert_eq_fmt(actual, expected, message, fmt)                       \
  do {                                                                         \
    ...
    if ((exp__) != (act__)) {                                                  \
      char _meqstr[MU_BUF_SIZE];                                               \
      snprintf(_meqstr, MU_BUF_SIZE, "%s: expected " fmt ", got " fmt ".",     \
               (message), (exp__), (act__));                                   \
      mu_assert(_meqstr, false);                                               \
    }                                                                          \
  } while (0)
...
#define mu_assert_streq(actual, expected, message)                             \
  do {                                                                         \
    ...
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected %s, got %s.", (message),      \
             (exp__), (act__));                                                \
...
#define mu_assert_strcontains(actual, expected, message)                       \
  do {                                                                         \
    ...
    const char *exp__ = (expected);                                            \
    exp__ = exp__ ? exp__ : "(null)";                                          \
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected to find %s in %s.",           \
             (message), (exp__), (act__));                                     \
...
#define mu_assert_nullable_streq(actual, expected, message)                    \
  do {                                                                         \
    ...
    const char *exp__ = (expected);                                            \
    snprintf(_meqstr, MU_BUF_SIZE, "%s: expected %s, got %s.", (message),      \
             (exp__ ? exp__ : "NULL"), (act__ ? act__ : "NULL"));              \
    mu_assert(_meqstr, ((act__) == NULL && (exp__) == NULL) ||                 \
                           ((act__) != NULL && (exp__) != NULL &&              \
                            strcmp((exp__), (act__)) == 0));                   \
  } while (0)
...
#define mu_assert_memeq(actual, expected, len, message)                        \
  do {                                                                         \
    ...
    snprint_mem(_meqstr + _meqstr_len, MU_BUF_SIZE - _meqstr_len, (exp__),     \
                (len));                                                        \
    _meqstr_len = strlen(_meqstr);                                             \
    snprintf(_meqstr + _meqstr_len, MU_BUF_SIZE - _meqstr_len, ", got ");      \
    _meqstr_len = strlen(_meqstr);                                             \
    snprint_mem(_meqstr + _meqstr_len, MU_BUF_SIZE - _meqstr_len, (act__),     \
                (len));                                                        \
...
#define mu_main(fcn)                                                           \
  int main(int argc, char **argv) { return fcn(); }
